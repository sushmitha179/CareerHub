...
export default function ResumeUpload({ initial, onChange }: Props) {
  ...
  async function removeResume() {
    ...
    try {
      const res = await fetch("/api/student/resume", { method: "DELETE" });
      const data = await res.json();

      if (!res.ok) {
        setStatus("error");
        setMessage(data.error || "Failed to remove resume");
        return;
      }
    ...
    <div className="border rounded-xl p-5 bg-gray-50 space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          PDF, DOC, or DOCX · max 5 MB · visible to companies you apply to
        </p>
      </div>
      ...
      {meta.hasResume ? (
          ...
          <div>
            ...
            {meta.resumeUploadedAt && (
              <p className="text-xs text-gray-500 mt-1">
                Uploaded{" "}
                {new Date(meta.resumeUploadedAt).toLocaleString()}
              </p>
            )}
          </div>
        ...
        >
          <p className="text-gray-600 mb-2">Drop or click to upload resume</p>
      ...
      {status === "deleting" && (
        <p className="text-sm text-gray-500">Removing resume...</p>
      )}
      {message && (
        <p
          className={`text-sm ${
